...
shinyUI(dashboardPage(dashboardHeader(title = "Basic Map Making Tool - Do Not Publish!!!"),
  dashboardSidebar(
    ...
    uiOutput("LAArea"),
    uiOutput("IndSelect"),
    conditionalPanel(condition = "input.MapT == 'Custom LA'",
                     sidebarMenu(menuItem(text = "Choose Centre", textInput("LAMap", "Choose Centre of Map", value = "Livingston")),
    menuItem(text = "Choose Zoom", sliderInput("MZoom", "Select Zoom", min = 5, max = 12, value = 9)))),
    textInput("LegendT", label = "Enter Legend Title", value = ""),
    sidebarMenu(menuItem(text = "Legend Controls", fluidRow(column(6, numericInput("LegX", "Legend Position X-axis", value = "0", min = 0, max = 1.1, step = 0.05)),
